...
%PROBLEM #2: Plotting system properties and determining damping
Cc = 1.6;
Cp = 0.2;
Lc = 0.015;
Rp = 1.1;
...
%PROBLEM #3: convolving an impulse train with a sine beat for replication
T = 1;
Ts = 0.3*sqrt(T);
dt1 = 0:0.001:(T-0.001);
ds = 0:0.001:(Ts-0.001);
...
%PROBLEM #4: Using lsim to determine output Pp(t)
...
%PROBLEM #5: Comparing steady state output Pc(t) to Pp(t) found above
...
%PROBLEM #6: Changing Stroke Volume, Beat Length, Central Compliance, and
% Peripheral Resistance to see the effects on Pc, Pp, Ql functions
