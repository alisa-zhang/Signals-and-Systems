...
%PROBLEM #1: Plotting Analytical FT of a shifted/scaled triang function
dw = 0:0.01:pi;
...
subplot(2,2,1);
plot(dw, Xreal);
title("Magnitude X(w) by analysis");
ylabel("Magnitude");
xlabel("Frequency");
...
Xim = (exp(1)).^(-12i.*dw);
plot(dw, angle(Xim));
title("Phase X(w) by analysis");
ylabel("Phase");
xlabel("Frequency");

%PROBLEM #2: Computing and plotting the Discrete Fourier Transform
Ts = 1;
x = triang(25);
DFT = fft(x, 256);
...
%PROBLEM #3: Varying the number of points in fft for future application
DFT16 = fft(x, 16);
DFT32 = fft(x, 32);
DFT128 = fft(x, 128);
...
%PROBLEM #5: Computing the DFT for both action potential signals
load("apdata.mat");
...
%PROBLEM #6: Comparing frequency plots of clean and noisy signals
subplot(2,2,3);
...
%PROBLEM #7: Creating a window-based filter for the noisy signal
figure;
subplot(4,3,1);
